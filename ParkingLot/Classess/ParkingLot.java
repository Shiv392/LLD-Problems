...
public class ParkingLot {
            ...
            try {
                for (ParkingSpot spot : parkingFloor.getParkingSpots()) {
                    ...
                    if (spot.isAvailable()) {
                        spot.park(vehicle);
                        System.out.println(
                                "Vechile " + vehicle.getName() + "Has been parked at spot id " + vehicle.getId());
                        return true;
                    }
                }
            } finally {
                parkingFloor.getLock().unlock();
            }
        }
    ...
    public boolean removeVehicle(int vehicleId, int floorId) {
        for (ParkingFloor parkingFloor : parkingFloors) {
            if (parkingFloor.getFloorId() != floorId)
                continue;
            ...
            try {
                ...
                if (vehicle != null && vehicle.getId() == vehicleId) {
                    spot.removeVehicle();
                    System.out.println(
                            "Vehicle with Id" + vehicleId + " " + vehicle.getName() + " has been removed from parking");
                    return true;
                }
            }
            } finally {
        ...
        for (ParkingFloor parkingFloor : parkingFloors) {
            ...
            for (ParkingSpot spot : parkingFloor.getParkingSpots()) {
                if (spot.isAvailable()) {
                    ...
                    sb.append(spot.getVehicle().getType() + " " + spot.getVehicle().getName() + " " + "Spot_Id: "
                            + spot.getSpotId() + " ");
                }
                System.out.println(sb.toString());
                sb = new StringBuilder();
